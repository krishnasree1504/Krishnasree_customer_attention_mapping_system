...
export class Database {
  ...
  createShelf(data: Partial<Shelf>): Shelf {
    const newShelf: Shelf = {
      ...
      storeId: data.storeId || 'str-1',
      storeName: data.storeName || 'Mumbai Central Flagship',
    ...
    const store = this.stores.find((s) => s.id === newShelf.storeId);
    if (store) store.shelfCount += 1;
  ...
  updateShelf(id: string, data: Partial<Shelf>): Shelf | undefined {
    ...
    this.shelves[idx] = {
      ...this.shelves[idx],
      ...data,
      name: data.shelfName || data.name || this.shelves[idx].name,
      shelfName: data.shelfName || data.name || this.shelves[idx].shelfName,
    };
  ...
  createCamera(data: Partial<Camera>): Camera {
    ...
    const store = this.stores.find((s) => s.id === newCamera.storeId);
    if (store) store.cameraCount += 1;
