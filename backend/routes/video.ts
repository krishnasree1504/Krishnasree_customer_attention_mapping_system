...
videoRouter.post('/analyze', async (req: Request, res: Response) => {
  try {
    ...
    try {
      await db.saveVideoAnalysis({
        id: job.jobId,
        videoFilename: job.originalName || path.basename(job.videoPath || 'video.mp4'),
        videoPath: job.videoPath,
        storeId: req.body.storeId || undefined,
        cameraId: req.body.cameraId || undefined,
        status: 'completed',
        totalPeople: analytics.totalPeople || analytics.totalFootfall || 0,
        uniquePeople: analytics.uniquePeople || 0,
        durationSec: Math.round(analytics.framesProcessed ? analytics.framesProcessed / (analytics.fps || 30) : 0),
      });
      await db.saveAnalysisResult(job.jobId, analytics);
      console.log(`[Video Route] Persisted analysis to PostgreSQL for ${job.jobId}`);
    } catch (dbErr) {
      console.warn('[Video Route] Warning: could not persist analysis to PostgreSQL:', dbErr);
    }
...
videoRouter.get('/result/:jobId', async (req: Request, res: Response) => {
  const { jobId } = req.params;
  ...
  try {
    const dbResult = await db.getAnalysisResult(cleanJobId);
    if (dbResult) {
      return res.json({ jobId: cleanJobId, status: 'completed', analytics: dbResult });
    }
  } catch (err) {
    console.warn('[Video Route] Error fetching analysis from DB:', err);
  }
...
const deleteJobHandler = async (req: Request, res: Response) => {
  ...
  try {
    await pool.query('DELETE FROM analysis_results WHERE analysis_id = $1', [cleanJobId]);
    await pool.query('DELETE FROM video_analyses WHERE id = $1', [cleanJobId]);
  } catch (dbDelErr) {
    console.warn('[Video Route] Error deleting analysis from DB:', dbDelErr);
  }
