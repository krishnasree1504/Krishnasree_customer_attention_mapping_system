...
export const VideoAnalysisSection: React.FC = () => {
  ...
  const processSelectedFile = (file: File) => {
    ...
    // Get Video Duration using temporary URL
  ...
  // 2. Analyze Video Action
  const handleAnalyzeVideo = async () => {
    let activeJobId = jobId;

    if (!activeJobId && selectedFile) {
      // Auto trigger upload if not uploaded yet
      try {
        setIsUploading(true);
        const formData = new FormData();
        formData.append('video', selectedFile);
    ...
    try {
      // Call backend API /api/video/analyze
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Upload & Control Card */}
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
        <input
          type="file"
          ref={fileInputRef}
          onChange={handleFileChange}
          accept="video/mp4,video/avi,video/quicktime,video/x-matroska,video/webm,.mp4,.avi,.mov,.mkv"
          className="hidden"
        />
        ...
        {!selectedFile ? (
          <div
            ...
            className="border-2 border-dashed border-slate-300 hover:border-[#00E676] bg-slate-50/50 hover:bg-[#00E676]/5 p-8 rounded-2xl text-center cursor-pointer transition-all group flex flex-col items-center justify-center gap-3"
          >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-[#00E676] hover:bg-[#00c865] text-slate-950 font-bold text-xs shadow-md shadow-[#00E676]/25 transition-all mt-2"
            >
              Browse Computer
            </button>
          </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-200">
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-bold text-xs shadow-md flex items-center gap-2 transition-all ${
                  !isAnalyzing
                    ? 'bg-[#00E676] hover:bg-[#00c865] text-slate-950 shadow-[#00E676]/25 cursor-pointer'
      ...
      {analytics && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 to-slate-800 text-white border border-slate-800 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <a
              href={`/api/video/report/${jobId}`}
              download={`CAMS_YOLOv8_Report_${jobId}.pdf`}
              className="px-5 py-2.5 rounded-xl bg-[#00E676] hover:bg-[#00c865] text-slate-950 font-extrabold text-xs shadow-md shadow-[#00E676]/25 flex items-center justify-center gap-2 transition-all shrink-0"
            >
          ...
          {/* KPI Analytics Grid (7 Required Cards) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                Unique Shoppers Tracked: <span className="font-bold text-slate-800">{analytics.uniquePeople}</span>
              </p>
            </div>
              ...
              <div>
                <span
                  className={`inline-block px-3 py-1 rounded-full text-xs font-extrabold border ${
                    analytics.crowdLevel === 'Low'
                      ? 'bg-emerald-100 text-emerald-800 border-emerald-300'
                      : analytics.crowdLevel === 'Moderate'
                      ? 'bg-amber-100 text-amber-800 border-amber-300'
                      : 'bg-rose-100 text-rose-800 border-rose-300'
                  }`}
                >
                  {analytics.crowdLevel} Density
                </span>
              </div>
              <p className="text-[11px] text-slate-500 mt-2">Spatial congestion indicator</p>
            </div>

            {/* Card 4: Maximum Crowd */}
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
              <div className="flex items-center justify-between text-slate-400 mb-2">
            ...
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm sm:col-span-2 lg:col-span-2">
              ...
              <div className="flex items-baseline gap-3">
                ...
                <span className="text-xs text-slate-500 font-medium">
                  ({analytics.framesProcessed} frames analyzed across {analytics.videoDuration})
                </span>
              </div>
